...
const docSlider = (function () {
  ...
  const f = {
    ...
    createPager: function () {
      ...
      const pageLength = document.querySelectorAll(".docSlider-inner > *")
        .length;
    ...
    setData: function () {
      ...
      d.buttons = document.querySelectorAll(
  ...
  const u = {
    ...
    keyup: function (e) {
      ...
      if (
        ((shift && / |Spacebar/.test(key)) ||
          (!shift && /ArrowUp|Up|PageUp/.test(key))) &&
        u.scrollEnd(page, "top")
      ) {
        to = d.now - 1;
      } else if (
        !shift &&
        / |Spacebar|ArrowDown|Down|PageDown/.test(key) &&
        u.scrollEnd(page, "bottom")
      ) {
        to = d.now + 1;
    ...
    wheel: function (e) {
      ...
      requestAnimationFrame(function () {
        ...
        for (let i = 0; i < elms.length; i++) {
          ...
          if (
            elm.classList.contains("docSlider-scroll") &&
            !u.scrollEnd(elm, distance)
          )
            return;
        }
